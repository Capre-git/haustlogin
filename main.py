...
class Tologin:
    # CONFIG
    server = '210.43.0.195'
    username = '161413130521'
    password = '19980603'
    CONTROLCHECKSTATUS = b'\x20'
    ADAPTERNUM = b'\x09'
    print(GetLocalIPByPrefix())
    host_ip = GetLocalIPByPrefix()
    # host_ip='172.17.126.73'
    IPDOG = b'\x01'
    host_name = 'GILIGILIEYE'
    PRIMARY_DNS = '119.29.29.29'
    dhcp_server = '172.17.126.254'
    ...
    def bind_nic(self):
        try:
            ...
            return get_ip_address(self, self.nic_name)
    ...
    #print('nic_name='+ nic_name)
    s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)

    # s.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    def bind(self):
        self.s.bind((self.bind_ip, 61440))
        self.s.settimeout(3)
    ...
    def challenge(self, svr, ran):
        while True:
            ...
            self.s.sendto(b"\x01\x02" + t + b"\x09" + b"\x00" * 15, (svr, 61440))
            try:
    ...
    def keep_alive_package_builder(number, random, tail, type=1, first=False):
        ...
        if type == 3:
            foo = b''.join([bytes([int(i)]) for i in self.host_ip.split('.')])  # host_ip
            # CRC
    ...
    def keep_alive2(self, *args):
        ...
        svr_num = 0
        packet = self.keep_alive_package_builder(svr_num, self.dump(ran), b'\x00' * 4, 1, True)
        while True:
            # log('[keep-alive2] send1', str(binascii.hexlify(packet))[2:][:-1])
            self.s.sendto(packet, (svr, 61440))
            data, address = self.s.recvfrom(1024)
        ...
        ran += random.randint(1, 10)
        packet = self.keep_alive_package_builder(svr_num, self.dump(ran), b'\x00' * 4, 1, False)
        ...
        ran += random.randint(1, 10)
        packet = self.keep_alive_package_builder(svr_num, self.dump(ran), tail, 3, False)
        ...
        while True:
            try:
                time.sleep(20)
                self.keep_alive1(*args)
                ran += random.randint(1, 10)
                packet = self.keep_alive_package_builder(i, self.dump(ran), tail, 1, False)
                ...
                ran += random.randint(1, 10)
                packet = self.keep_alive_package_builder(i + 1, self.dump(ran), tail, 3, False)
    ...
    def mkpkt(self, salt, usr, pwd, mac):
        '''
        ...
        data += self.md5sum(b'\x03\x01' + salt + pwd.encode())
        data += (usr.encode() + 36 * b'\x00')[:36]
        data += self.CONTROLCHECKSTATUS
        data += self.ADAPTERNUM
        data += self.dump(int(binascii.hexlify(data[4:10]), 16) ^ mac)[-6:]  # mac xor md51
        data += self.md5sum(b'\x01' + pwd.encode() + salt + b'\x00' * 4)  # md52
        data += b'\x01'  # number of ip
        data += b''.join([bytes([int(i)]) for i in self.host_ip.split('.')])  # x.x.x.x ->
        data += b'\00' * 4  # your ipaddress 2
        data += b'\00' * 4  # your ipaddress 3
        data += b'\00' * 4  # your ipaddress 4
        data += self.md5sum(data + b'\x14\x00\x07\x0B')[:8]  # md53
        data += self.IPDOG
        ...
        '''
        data += (self.host_name.encode() + 32 * b'\x00')[:32]  # _tagHostInfo.HostName
        data += b''.join([bytes([int(i)]) for i in self.PRIMARY_DNS.split('.')])  # _tagHostInfo.DNSIP1
        data += b''.join([bytes([int(i)]) for i in self.dhcp_server.split('.')])  # _tagHostInfo.DHCPServerIP
        ...
        struct  _tagLDAPAuth
        {
            ...
            data += self.ror(self.md5sum(b'\x03\x01' + salt + pwd), pwd)  # _tagLDAPAuth.Password
        '''
        ...
        data += self.checksum(data + b'\x01\x26\x07\x11\x00\x00' + self.dump(mac))  # _tagDrcomAuthExtData.CRC
        data += b'\x00\x00'  # _tagDrcomAuthExtData.Option
        data += self.dump(mac)  # _tagDrcomAuthExtData.AdapterAddress
    ...
    def login(self, usr, pwd, svr):
        i = 0
        while True:
            salt = self.challenge(svr, time.time() + random.randint(0xF, 0xFF))
            SALT = salt
            packet = self.mkpkt(salt, usr, pwd, self.mac)
            ...
            if address == (svr, 61440):
                if data[:1] == b'\x04':
                    ...
                    raise self.LogSuccessful("AMD yes")
                    # break
                else:
    ...
    def logout(self, usr, pwd, svr, mac, auth_info):
        salt = self.challenge(svr, time.time() + random.randint(0xF, 0xFF))
        if salt:
            ...
            data += self.CONTROLCHECKSTATUS
            data += self.ADAPTERNUM
            data += self.dump(int(binascii.hexlify(data[4:10]), 16) ^ mac)[-6:]
    ...
    def keep_alive1(self, salt, tail, pwd, svr):
        foo = struct.pack('!H', int(time.time()) % 0xFFFF)
        data = b'\xff' + self.md5sum(b'\x03\x01' + salt + pwd.encode()) + b'\x00\x00\x00'
        data += tail
    ...
    def main(self):
        ...
        while True:
            try:
                # bind()
                package_tail = self.login(self.username, self.password, self.server)
                break

            except self.LoginException:
                continue
            # log('package_tail', str(binascii.hexlify(package_tail))[2:][:-1])
            # keep_alive1 is fucking bullshit!
            empty_socket_buffer()
            keep_alive1(self.SALT, package_tail, self.password, self.server)
            keep_alive2(self.SALT, package_tail, self.password, self.server)
...
class LoginScreen(GridLayout):
    ...
    def widadder(self):
        self.add_widget(self.usrlabel)
        self.add_widget(self.username)
        self.add_widget(self.pwlabel)
        self.add_widget(self.password)
        self.add_widget(self.infodisplay[0])
        self.add_widget(self.infodisplay[1])
    def design(self):
        with self.usrlabel.canvas:
            Color(1., 1., 0)
            print(self.pos)
