...
class Tolongin:
    ...
    def keep_alive_package_builder(number, random, tail, type=1, first=False):
        global KEEP_ALIVE_VERSION, host_ip
    ...
    def mkpkt(self, salt, usr, pwd, mac):
        global CONTROLCHECKSTATUS,ADAPTERNUM,IPDOG,PRIMARY_DNS,dhcp_server,host_os,AUTH_VERSION,ror_version
        '''
    ...
    def login(self, usr, pwd, svr):
        global SALT
        global AUTH_INFO
        global mac,IS_TEST,UNLIMITED_RETRY
        i = 0
        while True:
            salt = self.challenge(svr, time.time() + random.randint(0xF, 0xFF))
            SALT = salt
            packet = self.mkpkt(salt, usr, pwd, mac)
